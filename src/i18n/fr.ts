import type { Dict } from './es';

export const fr: Dict = {
  lang: { es: 'Español', en: 'English', fr: 'Français', de: 'Deutsch', it: 'Italiano' },
  common: {
    back: 'Retour',
    locked: 'Verrouillé',
    loading: 'Chargement… {pct} %',
    loadError: 'Impossible de charger le jeu. Relance-le.',
    comingSoon: 'Bientôt',
    rotate: 'Tourne ton téléphone pour jouer en paysage',
    stars: '{n} étoiles sur 3',
  },
  menu: {
    subtitle: 'Donjons de Slime',
    hint: 'Glisse, divise et rassemble ton slime',
    story: 'Mode Histoire',
    myslime: 'Mon slime',
    shop: 'Boutique',
    profile: 'Profil',
    options: 'Options',
  },
  story: {
    title: 'Mode Histoire',
    chapter: 'Chapitre {n}',
    floor: 'Étage {n}',
    breakdown: 'Voir le bilan du chapitre',
    reward: 'À 100 % : {item}',
    rewardOwned: 'Récompense obtenue',
  },
  chapters: {
    'cripta-azul': 'La Crypte Bleue',
  },
  options: {
    title: 'Options',
    control: 'Commandes',
    joystick: 'Joystick',
    gyro: 'Gyroscope',
    neutral: 'Position neutre',
    calibrate: 'Calibrer',
    sound: 'Son',
    vibration: 'Vibration',
    on: 'Activé',
    off: 'Désactivé',
    language: 'Langue',
    practice: "Niveau d'entraînement",
    play: 'Jouer',
  },
  pause: {
    title: 'Pause',
    resume: 'Continuer',
    calibrate: "Calibrer l'inclinaison",
    restart: 'Recommencer',
    quit: 'Quitter',
  },
  hud: {
    pause: 'Pause',
    oiled: 'Huile',
    burning: 'En feu {s}',
    frozen: 'Gelé {s}',
  },
  toast: {
    secret: 'Trésor secret trouvé !',
    oil: "De l'huile ! Le feu va maintenant t'enflammer",
    ignite: 'En feu ! Brûle les plantes et la glace',
    freeze: "Gelé ! Les courants d'air te portent sans te briser",
    thaw: 'Ton slime a dégelé',
    extinguish: 'Les flammes se sont éteintes',
    reward: 'Nouvel accessoire : {item} !',
  },
  result: {
    done: 'Étage terminé !',
    failed: "Ton slime s'est désagrégé",
    failInfo: 'Il ne reste plus de slime',
    goalTreasure: 'Atteindre le trésor',
    goalCoins: 'Toutes les pièces',
    goalKeep: 'Garder {pct} % du slime',
    goalSecret: 'Trésor secret',
    found: 'Trouvé !',
    next: 'Étage suivant',
    toBreakdown: 'Voir le bilan du chapitre',
    retry: 'Rejouer',
    chapter: 'Chapitre',
    exit: 'Quitter',
  },
  breakdown: {
    done: '{chapter} terminé',
    progress: '{chapter} · progression',
    floor: 'Étage',
    stars: 'Étoiles',
    coins: 'Pièces',
    secret: 'Secret',
    slime: 'Slime',
    time: 'Temps',
    total: 'Total',
    gold: 'Rang Or',
    silver: 'Rang Argent',
    bronze: 'Rang Bronze',
    backChapter: 'Retour au chapitre',
    menu: 'Menu principal',
    perfect: 'Chapitre terminé à 100 % !',
  },
  shop: {
    title: 'Boutique',
    wallet: '{n} pièces',
    hint: "Gagne des pièces en les ramassant dans les étages. Les objets sont purement décoratifs : ils ne changent rien au jeu.",
    buy: 'Acheter · {n}',
    owned: 'Obtenu',
    equip: 'Porter',
    equipped: 'Porté',
    notEnough: 'Pas assez de pièces',
    rewardOnly: 'Récompense : termine {chapter} à 100 %',
  },
  myslime: {
    title: 'Mon slime',
    head: 'Tête',
    none: 'Aucun',
    empty: "Tu n'as pas encore d'accessoires : achète-les à la boutique ou termine un chapitre à 100 %.",
  },
  profile: {
    title: 'Profil',
    guest: 'Invité',
    signIn: 'Se connecter avec Google',
    signOut: 'Se déconnecter',
    connecting: 'Connexion…',
    notConfigured: "La connexion n'est pas encore configurée dans cette version.",
    error: 'Connexion impossible. Réessaie.',
    cloudSoon: 'Ta progression est enregistrée sur cet appareil. Bientôt : sauvegarde en ligne avec ton compte.',
    stats: 'Statistiques',
    starsTotal: 'Étoiles',
    coinsTotal: 'Pièces',
    secretsTotal: 'Secrets',
    chaptersDone: 'Chapitres à 100 %',
    items: 'Accessoires',
  },
  items: {
    hat_beanie: 'Bonnet de la Crypte',
    hat_leaf: 'Petite feuille',
    hat_bow: 'Nœud',
    hat_party: 'Chapeau de fête',
    hat_top: 'Haut-de-forme',
    hat_crown: 'Couronne',
  },
  levels: {
    'sendero-largo': {
      name: 'Long sentier (entraînement)',
      tips: ["Niveau d'entraînement : pas de pièges, tu ne perds pas de slime", "Descendre une marche ne fait pas mal", 'Longs couloirs : essaie des virages en douceur avec le joystick'],
    },
    'c1-primeros-pasos': {
      name: 'Premiers pas',
      tips: ['Fais glisser ton slime vers le trésor avec le joystick', 'Descendre une marche ne fait pas mal', 'Passage sans murs : ce qui dépasse du bord tombe', 'Contourne les trous', 'Ramasse toutes les pièces pour gagner une étoile'],
    },
    'c1-filo-de-cuchilla': {
      name: 'Fil de la lame',
      tips: ['Les lames coupent ton slime en deux', 'Les deux moitiés bougent en même temps', 'Quand les morceaux se touchent, ils se rassemblent', 'Les pointes le divisent aussi'],
    },
    'c1-pasillo-de-fuego': {
      name: 'Couloir de feu',
      tips: ['Pont étroit et sans murs : doucement', 'Le feu fait évaporer le slime et la glace glisse', "Ces flammes s'éteignent par moments : attends dans les zones sûres"],
    },
    'c1-divide-y-venceras': {
      name: 'Diviser pour régner',
      tips: ["La porte s'ouvre tant qu'un peu de slime reste sur l'interrupteur", "La lame envoie une partie dans la fosse et l'autre vers la porte"],
    },
    'c1-salto-al-abismo': {
      name: "Saut dans l'abîme",
      tips: ["Les plateformes roses te propulsent au-dessus de l'abîme", "Marche sur l'interrupteur pour ouvrir la sortie"],
    },
    'c1-aceite-y-chispas': {
      name: 'Huile et étincelles',
      tips: ["Prends la bouteille : ton slime devient de l'huile", "Avec l'huile, le feu ne t'évapore pas : il t'enflamme", 'En feu, tu brûles les plantes qui bloquent le passage'],
    },
    'c1-hielo-que-arde': {
      name: 'Glace brûlante',
      tips: ['Les flammes ne durent pas longtemps : dépêche-toi', 'En feu, tu fais fondre les blocs de glace'],
    },
    'c1-ventiladores': {
      name: 'Ventilateurs',
      tips: ['Le vent des ventilateurs disperse le slime vers le vide', 'Traverse vite les courants d’air'],
    },
    'c1-corriente-helada': {
      name: 'Courant glacé',
      tips: ["L'air froid gèle ton slime pendant 30 secondes", 'Gelé, il ne se disperse pas : le courant te porte en entier', 'Place-toi devant les ventilateurs'],
    },
    'c1-gran-cripta': {
      name: 'La grande crypte',
      tips: ["Dernière épreuve : utilise tout ce que tu as appris", 'Huile, feu… et brûle ces plantes'],
    },
  },
};
