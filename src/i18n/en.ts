import type { Dict } from './es';

export const en: Dict = {
  lang: { es: 'Español', en: 'English', fr: 'Français', de: 'Deutsch', it: 'Italiano' },
  common: {
    back: 'Back',
    locked: 'Locked',
    loading: 'Loading… {pct}%',
    loadError: 'Could not load the game files. Please restart the game.',
    comingSoon: 'Coming soon',
    rotate: 'Turn your phone sideways to play',
    stars: '{n} of 3 stars',
  },
  menu: {
    subtitle: 'Slime Dungeons',
    hint: 'Slide, split and regroup your slime',
    story: 'Story Mode',
    myslime: 'My slime',
    shop: 'Shop',
    profile: 'Profile',
    options: 'Options',
  },
  story: {
    title: 'Story Mode',
    chapter: 'Chapter {n}',
    floor: 'Floor {n}',
    breakdown: 'See chapter summary',
    reward: 'At 100%: {item}',
    rewardOwned: 'Reward unlocked',
  },
  chapters: {
    'cripta-azul': 'The Blue Crypt',
  },
  options: {
    title: 'Options',
    control: 'Controls',
    joystick: 'Joystick',
    gyro: 'Gyroscope',
    neutral: 'Neutral position',
    calibrate: 'Calibrate',
    sound: 'Sound',
    vibration: 'Vibration',
    on: 'On',
    off: 'Off',
    language: 'Language',
    practice: 'Practice level',
    play: 'Play',
  },
  pause: {
    title: 'Paused',
    resume: 'Resume',
    calibrate: 'Calibrate tilt',
    restart: 'Restart',
    quit: 'Quit',
  },
  hud: {
    pause: 'Pause',
    oiled: 'Oil',
    burning: 'On fire {s}',
    frozen: 'Frozen {s}',
  },
  toast: {
    secret: 'Secret treasure found!',
    oil: 'Oil! Fire will now set you ablaze',
    ignite: 'On fire! Burn through plants and ice',
    freeze: 'Frozen! Air currents carry you in one piece',
    thaw: 'Your slime has thawed',
    extinguish: 'The flames went out',
    reward: 'New accessory: {item}!',
  },
  result: {
    done: 'Floor cleared!',
    failed: 'Your slime fell apart',
    failInfo: 'There is no slime left',
    goalTreasure: 'Reach the treasure',
    goalCoins: 'All the coins',
    goalKeep: 'Keep {pct}% of your slime',
    goalSecret: 'Secret treasure',
    found: 'Found!',
    next: 'Next floor',
    toBreakdown: 'See chapter summary',
    retry: 'Retry',
    chapter: 'Chapter',
    exit: 'Exit',
  },
  breakdown: {
    done: '{chapter} complete',
    progress: '{chapter} · progress',
    floor: 'Floor',
    stars: 'Stars',
    coins: 'Coins',
    secret: 'Secret',
    slime: 'Slime',
    time: 'Time',
    total: 'Total',
    gold: 'Gold rank',
    silver: 'Silver rank',
    bronze: 'Bronze rank',
    backChapter: 'Back to chapter',
    menu: 'Main menu',
    perfect: 'Chapter 100% complete!',
  },
  shop: {
    title: 'Shop',
    wallet: '{n} coins',
    hint: 'Earn coins by collecting them on each floor. Items are cosmetic only: they do not change the game.',
    buy: 'Buy · {n}',
    owned: 'Owned',
    equip: 'Wear',
    equipped: 'Wearing',
    notEnough: 'Not enough coins',
    rewardOnly: 'Reward: complete {chapter} at 100%',
  },
  myslime: {
    title: 'My slime',
    head: 'Head',
    none: 'None',
    empty: 'You have no accessories yet: buy them in the shop or complete a chapter at 100%.',
  },
  profile: {
    title: 'Profile',
    guest: 'Guest',
    signIn: 'Sign in with Google',
    signOut: 'Sign out',
    connecting: 'Connecting…',
    notConfigured: 'Sign-in is not set up in this version yet.',
    error: 'Could not sign in. Please try again.',
    cloudSoon: 'Your progress is saved on this device. Coming soon: cloud backup with your account.',
    stats: 'Stats',
    starsTotal: 'Stars',
    coinsTotal: 'Coins',
    secretsTotal: 'Secrets',
    chaptersDone: 'Chapters at 100%',
    items: 'Accessories',
  },
  items: {
    hat_beanie: 'Crypt Beanie',
    hat_leaf: 'Little Leaf',
    hat_bow: 'Bow',
    hat_party: 'Party Hat',
    hat_top: 'Top Hat',
    hat_crown: 'Crown',
  },
  levels: {
    'sendero-largo': {
      name: 'Long Path (practice)',
      tips: ['Practice level: no traps, you cannot lose slime', 'Dropping down a ledge does no harm', 'Long corridors: try smooth turns with the joystick'],
    },
    'c1-primeros-pasos': {
      name: 'First Steps',
      tips: ['Slide your slime towards the treasure with the joystick', 'Dropping down a ledge does no harm', 'No walls here: whatever sticks out over the edge falls', 'Go around the holes', 'Collect every coin to earn a star'],
    },
    'c1-filo-de-cuchilla': {
      name: 'Blade Edge',
      tips: ['Blades split your slime in two', 'Both halves move at the same time', 'When the pieces touch, they merge again', 'Spikes split it too'],
    },
    'c1-pasillo-de-fuego': {
      name: 'Fire Corridor',
      tips: ['Narrow bridge with no walls: take it easy', 'Fire evaporates slime and ice is slippery', 'These flames go out now and then: wait in the safe areas'],
    },
    'c1-divide-y-venceras': {
      name: 'Divide and Conquer',
      tips: ['The door stays open while some slime is on the switch', 'The blade sends one part into the pit and the other to the door'],
    },
    'c1-salto-al-abismo': {
      name: 'Leap of Faith',
      tips: ['Pink pads launch you over the abyss', 'Step on the switch to open the exit'],
    },
    'c1-aceite-y-chispas': {
      name: 'Oil and Sparks',
      tips: ['Grab the bottle: your slime turns into oil', 'Oily slime does not evaporate in fire: it catches fire', 'While burning, you burn away the plants blocking the way'],
    },
    'c1-hielo-que-arde': {
      name: 'Burning Ice',
      tips: ['The flames do not last long: hurry', 'While burning, you melt the ice blocks'],
    },
    'c1-ventiladores': {
      name: 'Fans',
      tips: ['Fan wind blows your slime apart into the void', 'Cross the air currents quickly'],
    },
    'c1-corriente-helada': {
      name: 'Frozen Current',
      tips: ['Cold air freezes your slime for 30 seconds', 'Frozen slime stays in one piece: the current carries it', 'Stand in front of the fans'],
    },
    'c1-gran-cripta': {
      name: 'The Great Crypt',
      tips: ['Final test: use everything you have learned', 'Oil, fire… and burn those plants'],
    },
  },
};
